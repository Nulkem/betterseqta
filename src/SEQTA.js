...
function loading() {
  ...
  loadinghtml = stringToHTML(
    ...
    <div style="position: absolute;bottom: 0;right: 0;padding: 10px;color: #4f4f4f;text-anchor: middle;font-size: 20px;">v1.4</div></div>`
  );
...
document.addEventListener(
  "load",
  function () {
    if (document.childNodes[1].textContent.includes("Copyright (c) SEQTA Software") && !IsSEQTAPage) {
      IsSEQTAPage = true;
...
function AddBetterSEQTAElements(toggle) {
  ...
  if (code != null) {
    if (!code.innerHTML.includes("BetterSEQTA")) {
      code.innerHTML = "BetterSEQTA v1.4";
