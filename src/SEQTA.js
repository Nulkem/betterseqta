...
function loading() {
  ...
  loadinghtml = stringToHTML(
    ...
    <div style="position: absolute;bottom: 0;right: 0;padding: 10px;color: #4f4f4f;text-anchor: middle;font-size: 20px;">v1.47</div></div>`
  );
...
chrome.storage.onChanged.addListener(function (changes) {
  if (changes.selectedColor) {
    ...
    document.documentElement.style.setProperty('--better-light', ColorLuminance(changes.selectedColor.newValue, 0.95));
  }
})
...
function AddBetterSEQTAElements(toggle) {
  ...
  if (code != null) {
    if (!code.innerHTML.includes("BetterSEQTA")) {
      code.innerHTML = "BetterSEQTA v1.47";
...
function SendHomePage() {
  setTimeout(function () {
    ...
    var htmlStr =
      `<div class="home-root"><div class="home-container" id="home-container"><h1 class="border">${timeString}${FirstName}!</h1></div></div>`;
    ...
    var TimetableStr = `<div class="timetable-container border"><h2>Today's Lessons</h2><div class="day-container" id="day-container"></div></div>`;
    ...
    var NoticesStr = `<div class="notices-container border"><h2>Notices</h2><div class="notice-container" id="notice-container"></div></div>`;
